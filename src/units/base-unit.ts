 ...
 * - Each unit has a `toBaseFactor` that converts it to its component base unit
 * - Any conversion becomes: (value * fromUnit.toBaseFactor) / toUnit.toBaseFactor
 * - Units only convert inside the same connected conversion component
 */
export interface BaseUnit {
  ...
  /**
   * Conversion factor to the component base unit.
 ...
 * 
 * Uses hub-and-spoke conversion per connected unit component.
 */
export class UnitSystem {
  ...
  convert(value: number, fromUnit: string, toUnit: string): number | undefined {
    ...
    if ((from.baseUnitName ?? from.name) !== (to.baseUnitName ?? to.name)) {
      return undefined;
    }
